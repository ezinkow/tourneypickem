...
export default function SquaresGrid() {
    ...
    const rowNumbers = useMemo(() => {
        ...
        for (const s of assigned) rows[Math.floor(s.square_id / 10)] = s.rowNumber;
        return rows;
    }, [grid]);
    ...
    const ownerColors = useMemo(() => {
        const owners = [...new Set(grid.map(s => s.owner_name).filter(Boolean))];
        const palette = [
                        ...
                        <div style={{ display: "flex", alignItems: "center", gap: 12 }}>
                            ...
                            {myCount > 0 && (
                                <span style={{ fontSize: 13, color: "rgba(255,255,255,0.7)" }}>
                                    {myCount} square{myCount !== 1 ? "s" : ""} · {myCount * 25}
                                </span>
                            )}
            ...
            <div style={{ maxWidth: 900, margin: "0 auto", padding: "0 12px" }}>
                ...
                {!authenticated && !isLocked && (
                    <div style={{ display: "flex", gap: 8, alignItems: "center", marginBottom: 20, flexWrap: "wrap" }}>
                        ...
                        <button
                            onClick={() => window.location.hash = "#/squares/change-password"}
                ...
                <div style={{ overflowX: "auto", WebkitOverflowScrolling: "touch" }}>
                    <div style={{ width: gridWidth + GUTTER_W, position: "relative" }}>
                                ...
                                {Array.from({ length: 10 }).map((_, rowIdx) => (
                                    <div key={rowIdx} style={{ display: "flex", alignItems: "center", marginBottom: 2 }}>
                                        ...
                                        {Array.from({ length: 10 }).map((_, colIdx) => {
                                            const square = grid[rowIdx * 10 + colIdx];
                ...
                <div style={{ marginTop: 24 }}>
                    <h4 style={{ color: BLUE, marginBottom: 12 }}>Square Ownership</h4>
                        ...
                        {Object.entries(
                            ...
                            .map(([name, count]) => (
                                <div key={name} style={{
                                    ...
                                    {name} — {count} square{count !== 1 ? "s" : ""} ({count * 25})
                                </div>
                            ))
                        }
                        {claimedCount < 100 && (
